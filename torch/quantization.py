...
class Trainer:
    ...
    def __load_model(self, quant=False):
        ...
        if quant:
            net = models.GetACDNetQuantModel(input_len=self.opt.inputLength, nclass=10, sr=self.opt.sr, channel_config=state['config']).to(self.opt.device);
        else:
            net = models.GetACDNetModel(input_len=self.opt.inputLength, nclass=10, sr=self.opt.sr, channel_config=state['config']).to(self.opt.device);
