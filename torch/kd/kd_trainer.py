...
def parse_pair(v):
    # Already a pair (tuple/list of size 2)
    if isinstance(v, (tuple, list)) and len(v) == 2:
        return int(v[0]), int(v[1])

    # String like "(1,1)" or "1,1"
    if isinstance(v, str):
        v = v.strip().strip("()")
        parts = v.split(",")
        if len(parts) == 2:
    ...
    # Single integer → duplicate it
    if isinstance(v, int):
        return v, v

    raise ValueError(f"Invalid start_at/stop_after value: {v}")
...
if __name__ == '__main__':
    ...
    tpaths = {
    1:'acdnet_20khz_trained_model_fold4_91.00.pt',
    2:'acdnet_20khz_trained_model_fold4_91.00.pt',
    3:'acdnet_20khz_trained_model_fold4_91.00.pt',
    4:'acdnet_20khz_trained_model_fold4_91.00.pt',
    5:'acdnet_20khz_trained_model_fold4_91.00.pt'}
    ...
    t_start, f_start = parse_pair(opt.start_at)
    t_stop,  f_stop  = parse_pair(opt.stop_after)
    ...
    for t in range(t_start, t_stop+1):
        ...
        for s in range(f, f_stop+1):
            opt.split = s;
            #opt.teacher_path = 'torch/resources/pretrained_models/{}'.format(tpaths[s]);
            opt.teacher_path = 'torch/resources/pretrained_models/acdnet/{}'.format(tpaths[s]);
