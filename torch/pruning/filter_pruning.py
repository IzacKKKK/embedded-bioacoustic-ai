...
class PruningTrainer:
    def __init__(self, opt):
        ...
        self.opt.finetune_epoch_per_iteration = 2; #previoulsy 2
        self.opt.lr=0.001;
    ...
    def PruneAndTrain(self):
        ...
        #aqui
        state = torch.load(self.opt.model_path, map_location=self.opt.device)
        ch_conf = state.get('config', None)  # channel config saved with the model
        self.net = models.GetACDNetModel(channel_config=ch_conf).to(self.opt.device)
        self.net.load_state_dict(state['weight'])
        print(
        f"[RUN] Dataset={self.opt.dataset} | Fold={self.opt.split} | "
        f"PruneType={self.opt.prune_type} | "
        f"PruneRatio={self.opt.prune_ratio} | "
        f"Area={'FULL' if self.opt.prune_all else 'TFEB'} | "
        f"FT_epochs/iter={self.opt.finetune_epoch_per_iteration}")
        #aqui
