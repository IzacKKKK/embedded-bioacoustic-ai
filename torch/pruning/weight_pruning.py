...
class PruningTrainer:
    ...
    def PruneAndTrain(self):
        ...
        if len(file_paths) > 0 and os.path.isfile(file_paths[0]):
          ...
          # read the channel config that was saved with the model (if any)
          ch_conf = state.get('config', None)

          # build the model WITH that config, so its channels match the checkpoint
          net = models.GetACDNetModel(channel_config=ch_conf).to(self.device)

          net.load_state_dict(state['weight'])
          print('Model Loaded from: {}'.format(file_paths[0]))
        else:
          print('Model is not found at: {}'.format(self.opt.model_path))
          exit()
        #end
