...
def prune_layers(net, prune_targets, prune_all=True, device='cpu'):
    ...
    for layer_index, channel_index in prune_targets:
        ...
        if layer_index == 3 and conv.out_channels <= 32:
            ...
            try:
              target_layer = list(all_features[len(all_features)-3])[1]
              if isinstance(target_layer, torch.nn.Conv2d):
                kernel = target_layer.kernel_size[0]
              else:
                kernel = None
            except Exception:
                kernel = None
            ...
            if kernel is not None and kernel > 1 and layer_index == 3 and new_conv.out_channels < 64:
