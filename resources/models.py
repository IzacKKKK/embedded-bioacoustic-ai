...
class ACDNetV2(nn.Module):
    def __init__(self, input_length, n_class, sr, ch_conf=None):
        ...
        if self.ch_config is None:
            ...
            self.ch_config = [channels, channels*1, channels*1, channels*1, channels*1, channels*2, channels*2, channels*4, channels*4, channels*8, channels*8, n_class];
        ...
        #fcn = nn.Linear(fcn_no_of_inputs, 11);
        ...
        # Final spatial squeeze: safe regardless of current HxW
        tfeb_modules.append(nn.AdaptiveAvgPool2d((1, 1)));

        # Classifier
        tfeb_modules.extend([nn.Flatten(), fcn]);

        self.tfeb = nn.Sequential(*tfeb_modules); #here 
...
def GetACDNetModel(input_len=30225, nclass=11, sr=20000, channel_config=None):
...
class ACDNetQuant(nn.Module):
    ...
    def get_tfeb_pool_size_component(self, length):
        ...
        return c;
#30225
def GetACDNetQuantModel(input_len=30225, nclass=10, sr=20000, channel_config=None):
